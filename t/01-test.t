...
is( API::ISPManager::refs( undef ),   '',       $a);
is( API::ISPManager::refs( {} ),      'HASH',   $a );
is( API::ISPManager::refs( [] ),      'ARRAY',  $a );
is( API::ISPManager::refs( sub {} ),  'CODE',   $a );
is( API::ISPManager::refs( \$a ),     'SCALAR', $a );

$a = 'is_success';

ok(! API::ISPManager::is_success(), $a);
ok(! API::ISPManager::is_success( { error => {}, data => {} } ), $a);
ok(  API::ISPManager::is_success( { data  => {} } ), $a);
ok(! API::ISPManager::is_success( { } ), $a);

$a = 'get_data';

ok(! API::ISPManager::get_data(), $a);
ok(! API::ISPManager::get_data( { } ), $a);
ok(! API::ISPManager::get_data( { error => {}, data => {} } ), $a);
is_deeply( API::ISPManager::get_data( { data => { aaa => 'bbb' } } ), { aaa => 'bbb' }, $a);
